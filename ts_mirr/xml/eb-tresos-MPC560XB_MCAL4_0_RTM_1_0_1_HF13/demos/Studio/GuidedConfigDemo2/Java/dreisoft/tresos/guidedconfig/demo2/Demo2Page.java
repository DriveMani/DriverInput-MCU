 ...
 * All rights reserved
 *
 * NAME: test.tresos.plugin.guidedconfig.demo2.Demo2Page
 */
...
/**
 * An example for a single page wizard which contains a custom widget and some
 * parameters from the datamodel configuration.
 * 
 * <p>
...
public class Demo2Page extends AbstractPage {
	...
	protected void doCreateControls(Composite parent) {
		...
		SWTGroup customGroup = factory.createGroup(parent, "GroupWidget1",
				WidgetFactory.GroupDecoration.TITLE,
				WidgetFactory.GroupLayout.VERTICAL, MyEclipseNLS
						.TITLE_GROUP_DATETIME());

		//
		// Custom Widget
		// create Custom Datetime widget
		MyDatetime datetimeWidget = new MyDatetime(WIDGETID_DATETIME,
				customGroup);
		...
		MyDatetimeController datetimeController = new MyDatetimeController(
				datetimeWidget, "DatetimeMemento");
		...
		// Standard Widgets
		// create a collapseable group
		SWTGroup modelGroup = factory.createGroup(parent, "GroupWidget2",
				WidgetFactory.GroupDecoration.TITLE,
				WidgetFactory.GroupLayout.VERTICAL, MyEclipseNLS
						.TITLE_GROUP_DATAMODEL());

		// create a checkbox
		factory.createLabel(modelGroup, "LabelWidget1", "LabelMemento1",
				MyEclipseNLS.LABEL_BOOL());
		factory.createCheckBox(modelGroup, WIDGETID_CHECKBOX,
				IDemo2Constants.MEMENTO_CHECKBOX);

		// create spacer
		factory.createSpacer(modelGroup, 2, 1);

		// create a textbox of type integer
		factory.createLabel(modelGroup, "LabelWidget2", "LabelMemento2",
				MyEclipseNLS.LABEL_INT());
		factory.createText(modelGroup, WIDGETID_INT,
				IDemo2Constants.MEMENTO_INT, WidgetFactory.TextFormat.INTEGER);

		// create a textbox of type float
		factory.createLabel(modelGroup, "LabelWidget3", "LabelMemento3",
				MyEclipseNLS.LABEL_FLOAT());
		factory.createText(modelGroup, WIDGETID_FLOAT,
				IDemo2Constants.MEMENTO_FLOAT, WidgetFactory.TextFormat.FLOAT);

		// create a textbox
		factory.createLabel(modelGroup, "LabelWidget4", "LabelMemento4",
				MyEclipseNLS.LABEL_TEXT());
		factory.createText(modelGroup, WIDGETID_STRING,
				IDemo2Constants.MEMENTO_STRING, WidgetFactory.TextFormat.TEXT);
