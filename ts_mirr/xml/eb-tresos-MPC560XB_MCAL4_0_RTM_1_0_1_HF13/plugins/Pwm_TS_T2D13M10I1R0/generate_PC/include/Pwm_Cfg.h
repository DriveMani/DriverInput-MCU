...
*/
#define PWM_EMIOS_16_BITS_VARIANT   ([!"ecu:get('Pwm.Emios16BitsVariant')"!])
[!NOCODE!]
    ...
    [!LOOP "PwmChannelConfigSet/*/PwmChannel/*"!]
        ...
        [!LOOP "../../../*/PwmChannel/*"!]
            ...
            [!IF "$OuterLoopCounter >= $InnerLoopCounter"!]
                [!IF "contains(PwmModeSelect,'OPWMT')"!][!//
                    [!VAR "opwmtfeature" = "1"!]
                [!ELSEIF "contains(PwmModeSelect,'OPWMCB')"!]
                    [!VAR "opwmcbfeature" = "1"!]
                [!ELSEIF "contains(PwmModeSelect,'DAOC')"!]
                    [!VAR "daocfeature" = "1"!]
                [!ENDIF!]
            [!ENDIF!]
        [!ENDLOOP!]
...
*/
#define PWM_FEATURE_OPWMT  [!IF "$opwmtfeature = 1"!](STD_ON)[!ELSE!](STD_OFF)[!ENDIF!]
...
*/
#define PWM_FEATURE_OPWMCB [!IF "$opwmcbfeature = 1"!](STD_ON)[!ELSE!](STD_OFF)[!ENDIF!]
...
*/
#define PWM_FEATURE_DAOC   [!IF "$daocfeature = 1"!](STD_ON)[!ELSE!](STD_OFF)[!ENDIF!]
...
*/
#define MULTI_PWM_CHANNEL_SYNCH  ([!IF "PwmNonAUTOSAR/MultiPwmChannelSynch"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!])
...
*/
#define PWM_DUAL_CLOCK_MODE    ([!IF "PwmNonAUTOSAR/PwmEnableDualClockMode"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!])
...
*/
#define PWM_SETCOUNTERBUS_API         ([!IF "PwmNonAUTOSAR/PwmSetCounterBusApi"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!])
...
*/
#define PWM_SETCHANNELOUTPUT_API         ([!IF "PwmNonAUTOSAR/PwmSetChannelOutputApi"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!])
...
*/
#define PWM_DEV_ERROR_DETECT ([!IF "PwmGeneral/PwmDevErorDetect"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!])
...
* @implements DPWM03115
*/
#define PWM_CHANGE_REGISTER_A_SWITCH    ([!IF "PwmGeneral/PwmChangeRegisterA"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!])
...
*/
#define PWM_SET_TRIGGER_DELAY_API         ([!IF "PwmNonAUTOSAR/PwmSetTriggerDelay"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!])
...
* @violates @ref Pwm_Cfg_H_REF_3 This is required as per autosar Requirement, 
*/
#define PWM_DISABLE_DEM_REPORT_ERROR_STATUS   ([!IF "node:exists('PwmNonAUTOSAR/PwmDisableDemReportErrorStatus')"!][!IF "PwmNonAUTOSAR/PwmDisableDemReportErrorStatus"!]STD_OFF[!ELSE!]STD_ON[!ENDIF!][!ELSE!]STD_OFF[!ENDIF!])
...
*/
#define PWM_GET_CHANNEL_STATE    ([!IF "PwmNonAUTOSAR/PwmGetChannelState"!]STD_ON[!ELSE!]STD_OFF[!ENDIF!])
...
[!LOOP "PwmChannelConfigSet/*"!]
   [!VAR "current_max_channels"="num:i(count(PwmChannel/*))"!]
...
#define PwmConf_PwmChannel_[!"node:name(.)"!]         ((Pwm_ChannelType)[!"PwmChannelId"!])
[!ENDCODE!]
[!ELSE!]
[!ERROR!]
[!"node:name(.)"!] name is repeating in more than one Pwm Channel Config Set
[!ENDERROR!]
    [!ENDIF!]
    [!ENDLOOP!]
[!ENDNOCODE!]
...
[!LOOP "PwmChannelConfigSet/*/PwmChannel/*"!][!//
[!IF "contains($already_parsed_names, concat(node:name(.),'_',PwmHwChannel,'|')) = false()"!][!//
[!CODE!]
#define [!"node:name(.)"!]_[!"PwmHwChannel"!]    ((Pwm_ChannelType)[!"PwmHwChannel"!])
[!ENDCODE!]
[!VAR "already_parsed_names"="concat($already_parsed_names,node:name(.),'_',PwmHwChannel,'|')"!][!//
[!ENDIF!][!//
[!ENDLOOP!][!//
[!ENDNOCODE!]
/**@}*/
...
[!IF "PwmGeneral/PwmNotificationSupported"!][!// only generate the ISR macros if notifications are supported
    ...
    [!LOOP "PwmChannelConfigSet/*/PwmChannel/*"!]
        ...
        [!VAR "InnerLoopCounter" = "0"!]
        [!VAR "Matchcounter" = "0"!]
        [!VAR "hwChannel" = "PwmHwChannel"!][!// get the current HW channel

        [!LOOP "../../../*/PwmChannel/*"!][!// loop again from the start
            [!VAR "InnerLoopCounter" = "$InnerLoopCounter + 1"!][!// count channels
            [!IF "$OuterLoopCounter >= $InnerLoopCounter"!][!// only consider channels equal or below the current channel
                [!IF "($hwChannel = PwmHwChannel)"!]
                    [!VAR "Matchcounter" = "$Matchcounter + 1"!][!// increment on duplicate
                [!ENDIF!]
            [!ENDIF!]
        [!ENDLOOP!]

        [!IF "$Matchcounter = 1"!][!// only generate code if this is the last one w/o duplicates after - can be before but they are not considered
            [!CODE!]
/* @violates @ref Pwm_Cfg_H_REF_4 C macros shall only expand to a braced initialiser, a constant, a parenthesised expression */             
#define PWM_[!"PwmHwChannel"!]_ISR_USED[!ENDCODE!]
        [!ENDIF!]
...
*/
#define PWM_EMIOS_MODULES_CFG        ([!"ecu:get('Pwm.EmiosModules')"!]UL)
/**
* @brief emios channels available on EMIOS 0
* @implements DPWM03123
*/
#define PWM_EMIOS_A_HW_CHANNELS_CFG  ([!"ecu:get('Pwm.Emios0Channels')"!]UL)
...
[!NOCODE!][!VAR "var_count1"="num:i(count(PwmChannelConfigSet/*))"!][!VAR "w"="0"!][!ENDNOCODE!]
...
[!LOOP "PwmChannelConfigSet/*"!]
[!VAR "w"="$w + 1"!]
extern CONST(Pwm_ConfigType, PWM_CONST) [!"node:name(.)"!];
[!IF "$w < $var_count1"!] \
[!ENDIF!]
[!ENDLOOP!]
...
/* @violates @ref Pwm_Cfg_H_REF_6 C macros shall only expand to a braced, a constant, a parenthesised */
#define PWM_INIT_CONFIG_PC_DEFINE extern CONST(Pwm_ConfigType, PWM_CONST) Pwm_InitConfig_PC;
#endif /* PWM_PRECOMPILE_SUPPORT */
[!ENDIF!]
/*
* @violates @ref Pwm_Cfg_H_REF_3 This is required as per autosar Requirement, 
