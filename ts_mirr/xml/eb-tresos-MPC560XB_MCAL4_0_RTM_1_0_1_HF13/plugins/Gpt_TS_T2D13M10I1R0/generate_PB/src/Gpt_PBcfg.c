...
[!VAR "GptPrescConf_Alt"="(number(GptSTMChannelPrescale_Alternate))"!][!//
STATIC CONST(Gpt_LLD_ChannelConfigType, GPT_CONST) Gpt_InitChannelPB_[!"num:i(@index+1)"!][GPT_CHANNEL_NUM_[!"num:i(@index+1)"!]]=
{
[!VAR "STMErrorFlag" = "0"!][!//
...
[!IF "contains(GptHwChannel, 'RTC')"!][!//
        [!IF "GptChannelClkSrc = '0'"!]RTC_GPT_LLD_CLKSRC_32_KHZ[!ELSEIF "GptChannelClkSrc = '1'"!](uint8)RTC_GPT_LLD_CLKSRC_128_KHZ[!ELSE!]RTC_GPT_LLD_CLKSRC_16_MHZ[!ENDIF!], /* RTC clock source */[!//
[!ELSE!][!//
        (uint8)0, /* not used*/
[!ENDIF!][!//
[!NOCODE!]
...
[!ENDSELECT!][!//
[!IF "$x < num:i(count(GptChannelConfiguration/*)) - 1"!],[!ENDIF!]
[!ENDFOR!][!//
};
...
CONST(Gpt_ConfigType, GPT_CONST) [!"@name"!] =
{
    ...
    Gpt_InitChannelPB_[!"num:i(@index+1)"!]
};
