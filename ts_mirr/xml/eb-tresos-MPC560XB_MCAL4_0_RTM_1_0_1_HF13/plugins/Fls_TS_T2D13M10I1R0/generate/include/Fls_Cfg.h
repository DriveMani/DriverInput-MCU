...
#define FLS_OPT_TIMEOUT_VALUE        [!"FlsGeneral/FlsOperationTimeout"!]U

/* Timeout value for Abort operation */
#define FLS_ABT_TIMEOUT_VALUE        [!"FlsGeneral/FlsAbortTimeout"!]U
...
* @violates @ref Fls_Cfg_H_REF_2 Identifiers more than 31 characters.
*/
#define FlsConf_FlsGeneral_FlsDriverIndex_[!"num:i(FlsGeneral/FlsDriverIndex)"!]            [!"num:i(FlsGeneral/FlsDriverIndex)"!]U

/**
* @brief   Switches the Production Error Detection and Notification OFF
*/

#define FLS_DEM_ERROR_DETECT   [!IF "NonAutosar/FlsDisableDemReportErrorStatus ='false'"!](STD_ON)  /* Enable Production Error Detection */[!ELSE!](STD_OFF) /* Disable Production Error Detection */[!ENDIF!]
...
extern CONST(Dem_EventIdType, FLS_CONST)  Fls_E_CompareFailed;
[!ELSEIF "not(node:empty(./FlsDemEventParameterRefs/FLS_E_ERASE_FAILED))"!][!//
extern CONST(Dem_EventIdType, FLS_CONST)  Fls_E_EraseFailed;
[!ELSEIF "not(node:empty(./FlsDemEventParameterRefs/FLS_E_READ_FAILED))"!][!//
extern CONST(Dem_EventIdType, FLS_CONST)  Fls_E_ReadFailed;
[!ELSEIF "not(node:empty(./FlsDemEventParameterRefs/FLS_E_UNEXPECTED_FLASH_ID))"!][!//
extern CONST(Dem_EventIdType, FLS_CONST)  Fls_E_UnexpectedFlashId;
[!ELSEIF "not(node:empty(./FlsDemEventParameterRefs/FLS_E_WRITE_FAILED))"!][!//
extern CONST(Dem_EventIdType, FLS_CONST)  Fls_E_WriteFailed;
[!ENDIF!][!//
[!ENDLOOP!][!//
[!ENDIF!][!//
...
[!LOOP "FlsConfigSet/*"!][!//
/* Declaration of post-build structure ([!"node:name(.)"!])*/
/*
* @violates @ref Fls_Cfg_H_REF_3 Objects shall be defined at block scope if they are only accessed from within 
* a single function.
*/
extern CONST(Fls_ConfigType, FLS_CONST) [!"node:name(.)"!];
