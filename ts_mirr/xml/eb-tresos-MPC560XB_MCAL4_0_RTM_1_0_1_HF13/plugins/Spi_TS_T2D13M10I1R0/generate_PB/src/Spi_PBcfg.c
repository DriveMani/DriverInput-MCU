...
[!LOOP "SpiDriver/*"!]
    ...
    [!LOOP "SpiJob/*"!]
        [!LOOP "SpiChannelList/*"!]
            [!IF "contains(SpiChannelAssignment,$Current_ConfigSet)"!]
            [!ELSE!]
                [!ERROR!]
                            WRONG Reference is selected for SpiChannelAssignment in [!"../../@name"!]. Channels configured in [!"$Current_ConfigSet"!] should be referenced.
                [!ENDERROR!]
            [!ENDIF!]
        [!ENDLOOP!]

        [!IF "contains(SpiDeviceAssignment,$Current_ConfigSet)"!]
        [!ELSE!]
            [!ERROR!]
                        WRONG Reference is selected for SpiDeviceAssignment in [!"@name"!]. Devices configured in [!"$Current_ConfigSet"!] should be referenced.
            [!ENDERROR!]
        [!ENDIF!]
    [!ENDLOOP!]
    ...
    [!LOOP "SpiSequence/*"!]
        [!LOOP "SpiJobAssignment/*"!]
            ...
            [!ELSE!]
                [!ERROR!]
                            WRONG Reference is selected for SpiJobAssignment in [!"../../@name"!]. Jobs configured in [!"$Current_ConfigSet"!] should be referenced.
                [!ENDERROR!]
            [!ENDIF!]
        [!ENDLOOP!]
    [!ENDLOOP!]
...
#define SPI_VENDOR_ID_PBCFG_C                        43
 /*
* @violates @ref Spi_PBcfg_c_REF_5 This is required as per autosar Requirement, 
*/
...
#if ((SPI_AR_RELEASE_MAJOR_VERSION_PBCFG_C    != SPI_AR_RELEASE_MAJOR_VERSION_CFG) || \
     (SPI_AR_RELEASE_MINOR_VERSION_PBCFG_C    != SPI_AR_RELEASE_MINOR_VERSION_CFG) || \
     (SPI_AR_RELEASE_REVISION_VERSION_PBCFG_C != SPI_AR_RELEASE_REVISION_VERSION_CFG))
...
/* Check if current file and SPI header file are of the same Software version */
     ...
     (SPI_SW_PATCH_VERSION_PBCFG_C != SPI_SW_PATCH_VERSION_CFG))
...
/* [!"@name"!] Configuration */
/*
* @violates @ref Spi_PBcfg_c_REF_4 All declarations and definitions of objects or
...
[!IF "node:exists(node:ref(SpiDemEventParameterRefs/SPI_E_HARDWARE_ERROR))"!][!//
    { (uint32)STD_ON, [!"node:value(node:ref(SpiDemEventParameterRefs/SPI_E_HARDWARE_ERROR)/DemEventId)"!]U} /* SPI_E_HARDWARE_ERROR parameters*/ [!//
[!ELSE!][!//
